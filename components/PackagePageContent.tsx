...
const INCLUDED_ITEMS = [
  ...
  'Pickup & Drop from Kumbakonam',
  'Visit All 9 Navagraha Temples',
  ...
  return (
    <div className="bg-ivory">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          ...
          <div className="mt-10 flex flex-col gap-8 lg:grid lg:grid-cols-[minmax(0,1.85fr)_minmax(320px,1fr)] lg:items-start">
            <div className="min-w-0 lg:justify-self-start">
              ...
              <div className="hidden overflow-hidden rounded-xl border border-charcoal/20 bg-white shadow-soft md:block">
      ...
      {pkg.hasAccommodation && pkg.roomPricing && (
        <section className="py-16 lg:py-20">
          <div className="mx-auto max-w-4xl px-6 lg:px-10">
            <SectionLabel center>{lang === 'en' ? 'Accommodation Near Navagraha Temples' : 'நவக்கிரக கோவில்களுக்கு அருகில் தங்குமிடம்'}</SectionLabel>
...
function InclusionCard({
  ...
  return (
    <div className="flex h-full flex-col rounded-[24px] border border-gold/15 bg-white p-6 shadow-soft lg:min-h-[328px] lg:p-8">
      <div className="flex items-center gap-3 border-b border-gold/15 pb-5">
        <span className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-maroon/6">
          {icon}
        </span>
        <h3 className="font-heading text-2xl font-medium text-maroon">
          <span className="mr-2 text-gold">{symbol}</span>
          {title}
        </h3>
      </div>
      ...
      <ul className="mt-6 flex flex-1 flex-col justify-start gap-4">
        {items.map((item) => (
          <li key={item} className="flex items-start gap-3">
            ...
            <span className="font-body text-base leading-relaxed text-charcoal/80">{item}</span>
          </li>
        ))}
      </ul>
    </div>
